...
ax.text(450, 6, 'NSII', rotation=90, va='center', ha='left', fontsize=14)

pltVTS = ax.plot(450, 0.15*1000*3600, marker='h', color='black') # pixel PSF
ax.text(470., 0.13*1000*3600, "IACT PSF", fontsize=18)
ax.plot(420, 0.66, marker='.', color='black', markersize=12) # T14
ax.plot(420, 0.9, marker='.', color='black', markersize=12) # T13
ax.plot(420, 1.02, marker='.', color='black', markersize=12) # T23
ax.plot(420, 1.12, marker='.', color='black', markersize=12) # T24
ax.plot(420, 1.14, marker='.', color='black', markersize=12) # T12
ax.plot(420, 1.39, marker='.', color='black', markersize=12) # T34
VTS_logo = mpimg.imread('VERITAS_logo.png')
imagebox = OffsetImage(VTS_logo, zoom=0.1)
imagebox.image.axes = ax
ab = AnnotationBbox(offsetbox=imagebox, xy=(350, 0.5*(0.66+1.39)), frameon=False)
ax.add_artist(ab)
pltVTS = ax.fill_between(range(350, 600), [(3600*1000*180/np.pi)*1.22*x*1e-9/63.2 for x in range(350, 600)], [(3600*1000*180/np.pi)*1.22*x*1e-9/172.9 for x in range(350, 600)], interpolate=True, facecolor='purple', label='VTS-SII', alpha=0.5)
...
ax.text(x=540, y=0.1, s='asteroid occultations', fontsize=12)
...
ax.text(x=800, y=0.5, s='lunar occultations', va='bottom', fontsize=12)

pltCTAS = ax.fill_between(range(400, 600), [(3600*1000*180/np.pi)*1.22*x*1e-9/68 for x in range(400, 600)], [(3600*1000*180/np.pi)*1.22*x*1e-9/2503.8 for x in range(400, 600)], interpolate=True, facecolor='blue', alpha=0.15, label='CTA-S')
ax.text(400, 0.065, 'CTA-S', rotation=10, fontsize=12)
pltCTAN = ax.fill_between(range(400, 600), [(3600*1000*180/np.pi)*1.22*x*1e-9/71.56 for x in range(400, 600)], [(3600*1000*180/np.pi)*1.22*x*1e-9/854.7 for x in range(400, 600)], interpolate=True, facecolor='b', alpha=0.15, label='CTA-N')
ax.text(410, 0.2, 'CTA-N', rotation=10, fontsize=12)
...
#ax2.text(7e6, 0.08, "EVN+VLBA", rotation=30, fontsize=18)
...
plt.savefig(fname='VTS-AngResVsWavelength.jpg')
plt.show()
